(function() {
  ...
  BAMReader = (function() {
    ...
    BAMReader.prototype.toObject = function() {
      ...
      ret = {};
      _ref = ["size", "header", "header_offset", "refs", "bamfile", "cache_size", "nodic"];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        k = _ref[_i];
        ret[k] = this[k];
      }
...
(function() {
  ...
  BAMReader.prototype.createDic = function(op, callback) {
    ...
    return binarize = function($s, rstream) {
      var header_buf, idx_header, idx_header_str, l_count, read_write, remainder, three_byte_idx, tlen_dev, tlen_mean, tlen_n, tlen_sd, tlen_squared, tlen_sum, write_ended, wstream, _i, _len;
      tlen_sum = 0;
      tlen_squared = 0;
      tlen_n = 0;
      ...
      write_ended = false;
      read_write = function() {
        var buf, d, i, idx, line, lines, str, _j, _len1;
        if (write_ended) {
          return;
        }
        d = rstream.read();
      ...
      rstream.on("end", function() {
        var footer_buf, idx, offset, v, _j, _len1;
        footer_buf = new Buffer(256 * 256 * 256 * 7);
        offset = 0;
        for (idx = _j = 0, _len1 = three_byte_idx.length; _j < _len1; idx = ++_j) {
  ...
  BAMDic = (function() {
    ...
    BAMDic.prototype._read_footer = function() {
      ...
      while (i < footer_size) {
        ...
        total += num;
        this.three_byte_idx[idx3byte] = [total, num];
        i += 7;
      }
      return this.bufs = new module.exports.Fifo(1024 * 1024 * 400);
    };

    BAMDic.prototype.fetch = function(qname, d_offset_to_filter) {
      var bam, bams, buf, current, d_offset, delta, i_offset, idx, idxinfo, itr_count, left, line_i, lower, md5_buf, md5_o, newleft, newright, num, obi, offset, read_num, results, right, upper, _i, _j, _k, _len, _len1, _offset, _p, _ref;
      ...
      idxinfo = this.three_byte_idx[idx];
      if (!idxinfo) {
        return null;
      }
      read_num = idxinfo[1];
      if (!(buf = this.bufs.get(idx))) {
        ...
        fs.readSync(this.fd, buf, 0, DIC_SIZE * read_num, offset * DIC_SIZE + this.header_offset);
