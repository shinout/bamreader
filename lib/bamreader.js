(function() {
  ...
  BAMReader = (function() {
      ...
      childProcess.exec("which samtools", (function(_this) {
        return function(e, stdout, stderr) {
          if (_this.options.samtools) {
            return _this.runSamtools();
          }
          if (!_this.options.sam && (e || stderr || _this.options["native"])) {
            return _this.run();
          } else {
            return _this.runSamtools(_this.options.sam);
          }
        };
      })(this));
    }
    ...
    BAMReader.prototype.runSamtools = function(isSam) {
      ...
      if (this.bamfile.readable) {
        if (samtools) {
          return this.bamfile.pipe(samtools.stdin);
        }
      }
    };

    BAMReader.prototype.run = function() {
      var defBufOffset, deltaDefBuf, e, infBufOffset, onBam, onEnd, onHeader, onSam, options, reader, readingHeader, readstreamOption, refs, remainedDefBuf, remainedInfBuf, rstream, tlenJSON, _read;
      try {
      ...
      if (this.bamfile.readable) {
        ...
        if (typeof this.start === "number") {
          readstreamOption.start = this.start;
        }
        if (typeof this.end === "number") {
          readstreamOption.end = this.end;
        }
        rstream = fs.createReadStream(this.bamfile, readstreamOption);
      }
      refs = this.refs || {};
      readingHeader = this.refs ? false : true;
      remainedDefBuf = new Buffer(0);
      remainedInfBuf = new Buffer(0);
      defBufOffset = 0;
      infBufOffset = 0;
      deltaDefBuf = 0;
      _read = function(defBuf) {
        var bam, bufToInflate, bufsToInflate, headerInfo, headerStr, infBuf, infBufChunk, _i, _len, _ref, _ref1, _results;
        defBuf = Buffer.concat([remainedDefBuf, defBuf], remainedDefBuf.length + defBuf.length);
        _ref = BAMReader.splitDeflatedBuffer(defBuf), bufsToInflate = _ref[0], remainedDefBuf = _ref[1];
        _results = [];
        for (_i = 0, _len = bufsToInflate.length; _i < _len; _i++) {
          bufToInflate = bufsToInflate[_i];
          infBufChunk = inflateRawSync(bufToInflate);
          infBuf = remainedInfBuf.length ? Buffer.concat([remainedInfBuf, infBufChunk]) : infBufChunk;
          if (readingHeader) {
            try {
              headerInfo = BAMReader.readHeaderFromInflatedBuffer(infBuf, true);
              refs = headerInfo.refs, headerStr = headerInfo.headerStr, infBuf = headerInfo.infBuf;
              readingHeader = false;
              if (onHeader) {
                onHeader(headerStr);
              }
          ...
          while (true) {
            ...
            _ref1 = BAMReader.readAlignmentFromInflatedBuffer(infBuf, refs, reader), bam = _ref1[0], infBuf = _ref1[1];
            if (bam === null) {
              remainedInfBuf = infBuf;
              break;
            } else {
              if (onBam) {
                onBam(bam, defBufOffset, infBufOffset);
              }
          ...
          deltaDefBuf += bufToInflate.length + 26;
          if (remainedInfBuf.length === 0) {
            defBufOffset += deltaDefBuf;
            deltaDefBuf = 0;
            _results.push(infBufOffset = 0);
          } else {
    ...
    BAMReader.getDeflatedBuffer = function(fd, offset, nocheck) {
    ...
    BAMReader.readHeaderFromInflatedBuffer = function(infBuf, ifReturnsinfBuf) {
      var blen, cursor, headerLen, headerStr, i, nRef, name, nameLen, refLen, refs, ret, _i;
      refs = {};
      ...
      if (ifReturnsinfBuf) {
        ret.infBuf = infBuf.slice(cursor);
      }
      return ret;
    };

    BAMReader.readAlignmentFromInflatedBuffer = function(buf, refs, reader) {
      var Bam, arrayLen, bam, bin, blockSize, byte, char, cigar, cigarLen, cursor, e, flag, hLen, i, mapq, nextPos, nextRefId, num, pos, qual, readName, readNameLen, refId, rname, rnext, second, seq, seqBits, seqLen, seqLenByte, subtype, tLen, tag, tags, valtype, zLen, _i, _j, _len;
      try {
        cursor = 0;
        ...
        Bam = module.exports.Bam;
        bam = new Bam(reader || null, readName, flag, rname, pos + 1, mapq, cigar, rnext, nextPos + 1, tLen, seq, qual);
        bam.tags_ = tags;
